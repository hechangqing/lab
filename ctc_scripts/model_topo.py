...
if __name__ == '__main__':
    ...
    print '<BiLstm> <InputDim> ' + str(input_feat_dim) + ' <OutputDim> ' + str(lstm_cell_dim) + lstm_comm
    # the following bidirectional LSTM layers
    for n in range(1, lstm_layer_num):
         print '<BiLstm> <InputDim> ' + str(lstm_cell_dim) + ' <OutputDim> ' + str(lstm_cell_dim) + lstm_comm
    # the final affine-transform and softmax layer
    print '<AffineTransform> <InputDim> ' + str(lstm_cell_dim) + ' <OutputDim> ' + str(target_num) + ' <ParamStddev> ' + '0.004000'
